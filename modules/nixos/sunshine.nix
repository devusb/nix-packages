{ config, lib, pkgs, ... }:
let
  inherit (lib) mkEnableOption mkPackageOption mkOption mkIf types optionalString getExe;
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services.sunshine = {
      ...
      serviceConfig = {
        ...
        ExecStart = (if cfg.capSysAdmin then "${config.security.wrapperDir}/sunshine" else "${getExe cfg.package}")
